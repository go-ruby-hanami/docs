...
`go-ruby-hanami/hanami` reimplements Ruby's `hanami` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-hanami/hanami` directly, with no Ruby runtime;
...
Because `hanami` is pure Go with cgo disabled, it also compiles to **WebAssembly**
